...
def interwoven_grid(num_steps=20):
    """
    ...
    y_direction_first_part_y_vals = np.linspace(0.01, 0.99,num_steps, endpoint=False) 
    ...
    z_direction_first_part_z_vals = np.linspace(0.01, 0.99, num_steps, endpoint=False) 
    #I start and end a bit away from the ends, so that no two points are identitcal.
